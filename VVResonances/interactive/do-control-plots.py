...
def dataMC(data,mc,sig,var,postfix,cut,bins,mini,maxi,title,unit,leg1,leg2,leg3,logY=0):
    T = 0.08*H_ref
    B = 0.12*H_ref 
    L = 0.12*W_ref
    R = 0.04*W_ref
    ...
    hData=data.drawTH1(var,cut,lumi,bins,mini,maxi,title,unit,"HIST")
    hMC=mc.drawTH1(var,cut,lumi,bins,mini,maxi,title,unit,"HIST")
    hSig=sig.drawTH1(var,cut,lumi,bins,mini,maxi,title,unit,"HIST")
    hSig.Scale((hMC.Integral()/hSig.Integral()))
    ...
    legend.AddEntry(hData,leg1,"LEP")
    legend.AddEntry(hMC,leg2,"F")
    legend.AddEntry(hSig,leg3,"F")
    legend.Draw()
    ...
    return canvas,hData,hMC,legend #,pt
...
QCDherwig.setFillProperties(1001,921)

DATAPlotters = getPlotters(dataTemplate,True)
data=MergedPlotter(DATAPlotters)

SigPlotters = getPlotters(BulkGravWWTemplate,False)
sig = MergedPlotter(SigPlotters)

sig.setFillProperties(3001,36)
...
for p in purities:
	...
	c1 = dataMC(QCDherwig,QCDherwig,sig,'jj_l1_softDrop_mass',postfix,cut,40,55.,215.,'V candidate softdrop mass',"GeV","QCD","Data","G_{Bulk}(2 TeV)#rightarrowWW")
	...
	# c2 = compareSigQCD(QCD,sig,'jj_l2_tau21',postfix,cut,cut,20,0.,1.,'q candidate #tau_{21}',"","QCD","q*(2 TeV)#rightarrowqW")
	# c2[0].SaveAs(directory+"/"+c2[0].GetName()+".png")
	# c3 = compareSigQCD(QCD,sig,'jj_l1_softDrop_mass',postfix,cut,cut,60,0.,300.,'V candidate mass',"GeV","QCD","q*(2 TeV)#rightarrowqW")
	# c3[0].SaveAs(directory+"/"+c3[0].GetName()+".png")
	# c4 = compareSigQCD(QCD,sig,'jj_l2_softDrop_mass',postfix,cut,cut,60,0.,300.,'q candidate mass',"GeV","QCD","q*(2 TeV)#rightarrowqW")
	# c4[0].SaveAs(directory+"/"+c4[0].GetName()+".png")
	# c5 = compareSigQCD(QCD,sig,'jj_LV_mass',postfix,cut,cut,80,1000.,8000.,'M_{qV}',"GeV","QCD","q*(2 TeV)#rightarrowqW",1)
	# c5[0].SaveAs(directory+"/"+c5[0].GetName()+".png")
	# c6 = compareSigQCD(QCD,sig,'abs(jj_l1_eta-jj_l2_eta)',postfix,cut,cut,13,0.,1.3,'#Delta#eta(q,V)',"","QCD","q*(2 TeV)#rightarrowqW")
	# c6[0].SaveAs(directory+"/"+c6[0].GetName()+".png")
	# c7 = compareSigQCD(QCD,sig,'jj_l1_pt',postfix,cut,cut,50,200.,5000.,'V candidate p_{T}',"GeV","QCD","q*(2 TeV)#rightarrowqW",1)
	# c7[0].SaveAs(directory+"/"+c7[0].GetName()+".png")
	# c8 = compareSigQCD(QCD,sig,'jj_l2_pt',postfix,cut,cut,50,200.,5000.,'q candidate p_{T}',"GeV","QCD","q*(2 TeV)#rightarrowqW",1)
